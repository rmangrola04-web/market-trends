...
<head>
  ...
  <title>Options Pro Terminal - AI & Paper Trading</title>
...
<body class="bg-slate-950 text-slate-100 min-h-screen font-sans antialiased">
  ...
  <div id="loginOverlay" class="flex items-center justify-center min-h-screen px-4">
    <div class="bg-slate-900 border border-slate-800 p-8 rounded-2xl w-full max-w-md shadow-2xl">
      <div class="flex items-center justify-center mb-6">
        ...
        <h1 class="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-indigo-400">AI Trading Terminal</h1>
      </div>
      ...
      <form id="authForm" class="space-y-4">
        ...
        <button id="authSubmitBtn" type="submit" class="w-full bg-sky-600 hover:bg-sky-500 font-bold py-2.5 rounded-lg transition">Enter System</button>
      </form>
    </div>
  </div>
  ...
  <div id="mainTerminal" class="hidden max-w-7xl mx-auto p-4 space-y-6">
    <header class="flex flex-wrap justify-between items-center border-b border-slate-800 pb-4 gap-4">
      ...
      <div class="relative w-full md:w-80">
        <div class="flex items-center bg-slate-900 border border-slate-700 rounded-xl px-3 py-2">
          ...
          <input id="searchBar" type="text" placeholder="Search any Stock (e.g. ZOMATO, NIFTY)..." oninput="handleSearch(this.value)" class="bg-transparent text-xs w-full focus:outline-none">
        </div>
      ...
      <div class="flex items-center gap-3">
        <div id="connectionBadge" class="text-xs font-semibold px-3 py-1.5 rounded-lg bg-rose-500/20 text-rose-400 border border-rose-500/30">Broker: Unlinked ✕</div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
      <div class="bg-slate-900 border border-slate-800 p-3 rounded-xl"><div class="text-[10px] text-slate-400">Current Stock</div><div id="dispSymbol" class="text-sm font-bold text-indigo-400">--</div></div>
      <div class="bg-slate-900 border border-slate-800 p-3 rounded-xl"><div class="text-[10px] text-slate-400">Live Price</div><div id="dispPrice" class="text-sm font-bold">₹ --</div></div>
      ...
      <div class="bg-indigo-900/30 border border-indigo-500/50 p-3 rounded-xl"><div class="text-[10px] text-indigo-300">🤖 AI Momentum</div><div id="dispSignal" class="text-xs font-bold text-indigo-400">ANALYZING...</div></div>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2 bg-slate-900 border border-slate-800 p-4 rounded-2xl shadow-xl" id="chartWrapper">
        <div class="flex justify-between items-center mb-2">
          <span class="text-xs font-bold text-slate-300">Technical Chart (Multi-Screen)</span>
          <div class="flex gap-2 items-center">
            <select id="chartLayoutSelect" onchange="changeChartLayout(this.value)" class="bg-slate-800 text-[10px] px-2 py-1 rounded border border-slate-700 text-slate-300 outline-none">
              <option value="1">1 Chart</option>
              <option value="2">2 Charts</option>
              <option value="4">4 Charts</option>
            </select>
            <button onclick="toggleFullscreen()" class="bg-slate-700 hover:bg-slate-600 text-[10px] px-2 py-1 rounded transition">⛶ Fullscreen</button>
          </div>
        </div>
        <div id="chartGrid" class="grid grid-cols-1 gap-2 h-[450px] w-full rounded-lg overflow-hidden transition-all"></div>
      </div>

      <div class="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-xl flex flex-col justify-between">
        <div>
          <h2 class="text-sm font-bold text-emerald-400 mb-3">Options Buying (CE / PE)</h2>
          <div class="space-y-4">
            <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div class="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-xl">
        <h2 class="text-sm font-bold text-sky-400 mb-3">Broker Gateway & Paper Trading</h2>
        <select id="brokerSelect" onchange="renderBrokerForm()" class="w-full bg-slate-800 text-xs px-3 py-2 rounded-lg border border-slate-700 mb-3 font-semibold">
          <option value="paper">Paper Trading (Virtual) - Recommended</option>
          <option value="dhan">Dhan</option>
          <option value="angelone">Angel One (SmartAPI)</option>
        </select>
        <div id="brokerFields" class="space-y-2 mb-3"></div>
        <div class="flex gap-2">
          <button onclick="linkSelectedBroker()" class="w-1/2 bg-sky-600 py-2 rounded-lg text-xs font-bold hover:bg-sky-500">Link Broker</button>
          <button onclick="unlinkSelectedBroker()" class="w-1/2 bg-slate-700 py-2 rounded-lg text-xs font-bold hover:bg-slate-600">Unlink Broker</button>
        </div>
      </div>

      <div class="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-xl flex flex-col justify-between">
        <div>
            <h2 class="text-sm font-bold text-indigo-400 mb-3 uppercase">🤖 AI & Algo Auto-Entry</h2>
            ...
            <div class="space-y-2 mb-3">
                <div class="flex items-center justify-between p-2.5 bg-slate-800/60 rounded-lg border border-slate-700/50">
                    <span class="text-xs font-bold">PDH/PDL Basic Algo:</span>
                    <input id="riskAutoToggle" type="checkbox" class="w-5 h-5 accent-emerald-500 cursor-pointer">
                </div>
                <div class="flex items-center justify-between p-2.5 bg-indigo-900/20 rounded-lg border border-indigo-500/30">
                    <span class="text-xs font-bold text-indigo-300">🤖 AI Smart Mode (Trend + RSI):</span>
                    <input id="aiAutoToggle" type="checkbox" class="w-5 h-5 accent-indigo-500 cursor-pointer">
                </div>
            </div>
        </div>
        <button onclick="saveRiskSettings()" class="w-full bg-indigo-600 hover:bg-indigo-500 text-xs font-bold py-2.5 rounded-lg transition">Save Settings</button>
      </div>
    </div>
    
    <div class="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-xl mt-6">
  ...
  <script>
    ...
    setInterval(() => { document.getElementById("clock").innerText = new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" }); }, 1000);
    ...
    function switchAuthTab(mode) {
      ...
      if (isSignUpMode) {
        ...
        submitBtn.className = "w-full bg-emerald-600 hover:bg-emerald-500 font-bold py-2.5 rounded-lg transition";
        ...
        submitBtn.className = "w-full bg-sky-600 hover:bg-sky-500 font-bold py-2.5 rounded-lg transition";
    ...
    async function handleSearch(query) {
      ...
      if (res.ok) {
        ...
        else box.innerHTML = list.map(i => `<div onclick="selectSymbol('${i.symbol}')" class="p-2.5 hover:bg-slate-800 cursor-pointer border-b border-slate-800/50 flex justify-between items-center"><span class="text-xs font-bold text-white">${i.name} <span class="text-[10px] text-slate-400 font-mono ml-1">(${i.symbol})</span></span><span class="text-[9px] px-2 py-0.5 rounded bg-slate-800 text-sky-400 font-bold border border-slate-700">${i.segment}</span></div>`).join("");
    ...
    function getTVSymbol(sym) {
        ...
        if (sym === "CL=F") tvSymbol = "TVC:USOIL"; else if (sym === "GC=F") tvSymbol = "TVC:GOLD"; else if (sym === "SI=F") tvSymbol = "TVC:SILVER";
        else if (sym === "NG=F") tvSymbol = "TVC:NATURALGAS"; else if (sym === "^NSEI") tvSymbol = "NSE:NIFTY"; else if (sym === "^NSEBANK") tvSymbol = "NSE:BANKNIFTY";
        else if (sym === "NIFTY_FIN_SERVICE.NS") tvSymbol = "NSE:FINNIFTY"; else if (sym === "^BSESN") tvSymbol = "BSE:SENSEX";
    ...
    function renderTradingViewChart(sym, forceLayoutUpdate = false) {
      ...
      if(currentChartLayout === 1) grid.className = "grid grid-cols-1 gap-2 h-[450px] w-full rounded-lg overflow-hidden transition-all";
      else if(currentChartLayout === 2) grid.className = "grid grid-cols-1 md:grid-cols-2 gap-2 h-[450px] w-full rounded-lg overflow-hidden transition-all";
      else grid.className = "grid grid-cols-2 gap-2 h-[450px] w-full rounded-lg overflow-hidden transition-all";

      const defaultSymbols = [sym, "^NSEBANK", "^BSESN", "CL=F"];
      for(let i=0; i < currentChartLayout; i++) {
          let s = getTVSymbol(defaultSymbols[i] || sym);
          grid.innerHTML += `<iframe style="width: 100%; height: 100%; border: none; border-radius: 4px;" src="https://s.tradingview.com/widgetembed/?frameElementId=tv_${i}&symbol=${encodeURIComponent(s)}&interval=5&hidesidetoolbar=0&symboledit=1&saveimage=0&toolbarbg=131722&studies=[]&theme=dark&style=1&timezone=Asia%2FKolkata&locale=in"></iframe>`;
      }
    }

    function renderBrokerForm() {
      const b = document.getElementById("brokerSelect").value, f = document.getElementById("brokerFields");
      if (b === "paper") f.innerHTML = `<div class="p-3 bg-emerald-900/30 border border-emerald-500/50 rounded-lg text-xs text-emerald-400">✅ Paper Trading Mode: No API keys required. Click 'Link Broker' to start.</div>`;
      else if (b === "dhan") f.innerHTML = `<input id="f_cid" placeholder="Dhan Client ID" class="w-full bg-slate-800 text-xs px-3 py-2 rounded-lg border border-slate-700 mb-2 focus:outline-none focus:border-sky-500"><input id="f_tok" type="password" placeholder="Dhan Access Token" class="w-full bg-slate-800 text-xs px-3 py-2 rounded-lg border border-slate-700 focus:outline-none focus:border-sky-500">`;
      else f.innerHTML = `<input id="f_cid" placeholder="Angel Client Code" class="w-full bg-slate-800 text-xs px-3 py-2 rounded-lg border border-slate-700 mb-2 focus:outline-none focus:border-sky-500"><input id="f_key" placeholder="SmartAPI Key" class="w-full bg-slate-800 text-xs px-3 py-2 rounded-lg border border-slate-700 mb-2 focus:outline-none focus:border-sky-500"><input id="f_pin" type="password" placeholder="MPIN" class="w-full bg-slate-800 text-xs px-3 py-2 rounded-lg border border-slate-700 mb-2 focus:outline-none focus:border-sky-500"><input id="f_totp" type="password" placeholder="TOTP Secret Key" class="w-full bg-slate-800 text-xs px-3 py-2 rounded-lg border border-slate-700 focus:outline-none focus:border-sky-500">`;
    }
    ...
    async function fetchStatus() {
      ...
      try{
        ...
        if(res.ok){
            ...
            b.className = d.is_connected ? "text-xs font-semibold px-3 py-1.5 rounded-lg bg-emerald-500/20 text-emerald-400 border border-emerald-500/30" : "text-xs font-semibold px-3 py-1.5 rounded-lg bg-rose-500/20 text-rose-400 border border-rose-500/30";
            b.innerText = d.is_connected ? `Broker: ${d.active_broker} (${d.broker_display_id}) ✓` : "Broker: Unlinked ✕";
        ...
        for username, state in TERMINAL_STATES.items():
            if (state.get("auto_trade") or state.get("ai_mode")) and state.get("is_connected"):
                ...
                try:
                    ...
                    # Log Prefix based on broker type
                    pfx = "[PAPER TRADING] " if broker == "PAPER" else ""

                    if state.get("ai_mode"):
                        if cp > pdh and cp > sma and rsi > 55 and state["algo_position"] != "CE":
                            state["algo_position"] = "CE"
                            state["logs"].append(f"{datetime.now().strftime('%H:%M:%S')} - 🚀 {pfx}🤖 [AI] Bullish Trend (RSI:{rsi}) -> BUY CE ({symbol})")
                        elif cp < pdl and cp < sma and rsi < 45 and state["algo_position"] != "PE":
                            state["algo_position"] = "PE"
                            state["logs"].append(f"{datetime.now().strftime('%H:%M:%S')} - 🚀 {pfx}🤖 [AI] Bearish Trend (RSI:{rsi}) -> BUY PE ({symbol})")
                            
                    elif state.get("auto_trade") and not state.get("ai_mode"):
                        if cp > pdh and state["algo_position"] != "CE":
                            state["algo_position"] = "CE"
                            state["logs"].append(f"{datetime.now().strftime('%H:%M:%S')} - ⚡ {pfx}[ALGO] PDH Breakout -> BUY CE ({symbol})")
                        elif cp < pdl and state["algo_position"] != "PE":
                            state["algo_position"] = "PE"
                            state["logs"].append(f"{datetime.now().strftime('%H:%M:%S')} - ⚡ {pfx}[ALGO] PDL Breakdown -> BUY PE ({symbol})")
                except Exception as e:
                    pass

# ==========================================
# 5. APP INIT & DYNAMIC SYMBOLS
# ==========================================
@asynccontextmanager
...
app = FastAPI(title="Option AI Terminal Pro", lifespan=lifespan)
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_credentials=True, allow_methods=["*"], allow_headers=["*"])

# Expanded base list of top stocks, more will be dynamically fetched!
MASTER_SYMBOLS = [
    ...
    {"symbol": "GC=F", "name": "GOLD", "segment": "COMMODITY", "step": 100},
    {"symbol": "RELIANCE.NS", "name": "Reliance Ind", "segment": "EQUITY", "step": 20},
    {"symbol": "HDFCBANK.NS", "name": "HDFC Bank", "segment": "EQUITY", "step": 10},
    {"symbol": "ICICIBANK.NS", "name": "ICICI Bank", "segment": "EQUITY", "step": 10},
    {"symbol": "SBIN.NS", "name": "State Bank of India", "segment": "EQUITY", "step": 5},
    {"symbol": "INFY.NS", "name": "Infosys", "segment": "EQUITY", "step": 10},
    {"symbol": "TCS.NS", "name": "TCS", "segment": "EQUITY", "step": 20},
    {"symbol": "ITC.NS", "name": "ITC Ltd", "segment": "EQUITY", "step": 5},
    {"symbol": "LT.NS", "name": "Larsen & Toubro", "segment": "EQUITY", "step": 20},
    {"symbol": "BAJFINANCE.NS", "name": "Bajaj Finance", "segment": "EQUITY", "step": 50},
    {"symbol": "TATAMOTORS.NS", "name": "Tata Motors", "segment": "EQUITY", "step": 5},
    {"symbol": "ZOMATO.NS", "name": "Zomato", "segment": "EQUITY", "step": 1},
    {"symbol": "SUZLON.NS", "name": "Suzlon Energy", "segment": "EQUITY", "step": 1},
    {"symbol": "TATASTEEL.NS", "name": "Tata Steel", "segment": "EQUITY", "step": 2},
]
...
def get_user_state(username: str):
    if username not in TERMINAL_STATES:
        TERMINAL_STATES[username] = {
            ...
            "logs": [f"{datetime.now().strftime('%H:%M:%S')} - System and AI Engine ready."],
...
def search(q: str): 
    ...
    results = [s for s in MASTER_SYMBOLS if q.lower() in s["symbol"].lower() or q.lower() in s["name"].lower()][:10]
    
    # DYNAMIC SEARCH LOGIC (Lets user search ANY stock on Yahoo Finance)
    if len(q) > 2 and not any(q.upper() in r["symbol"].upper() for r in results):
        sym = q.upper() if q.upper().endswith(('.NS', '.BO', '=F')) else f"{q.upper()}.NS"
        results.append({"symbol": sym, "name": f"Search NSE: {sym}", "segment": "DYNAMIC", "step": 10})
...
def market_data(symbol: str, username: str = Depends(get_current_user)):
    try:
        df = yf.Ticker(symbol).history(period="10d", interval="5m")
        if df.empty or len(df) < 20: raise ValueError("Failed to fetch live data. Invalid symbol.")
        df_d = yf.Ticker(symbol).history(period="5d", interval="1d")
        cp = round(float(df["Close"].iloc[-1]), 2)
        pdh = round(float(df_d.iloc[-2]["High"]), 2) if len(df_d) >= 2 else cp
        pdl = round(float(df_d.iloc[-2]["Low"]), 2) if len(df_d) >= 2 else cp
    ...
    except Exception as e: raise HTTPException(400, f"Market data error: {str(e)}")
...
def link(req: BrokerReq, username: str = Depends(get_current_user)):
    ...
    try:
        if req.broker == "paper":
            ...
            state["logs"].append(f"{datetime.now().strftime('%H:%M:%S')} - Paper Trading Mode Activated.")
            return {"message": "Paper Trading Activated! No API required."}
            
        elif req.broker == "dhan": BROKER_INSTANCES[username] = dhanhq(req.client_id, req.access_token)
            ...
            if not smart.generateSession(req.client_id, req.pin, totp_code).get("status"): raise Exception("Angel One Login Failed")
    ...
    except Exception as e: raise HTTPException(400, f"Broker link error: {str(e)}")
...
def buy_order(req: OrderReq, username: str = Depends(get_current_user)):
    state = get_user_state(username)
    if not state["is_connected"]: raise HTTPException(400, "Please link broker first!")
    ...
    try:
        if broker == "PAPER":
            ...
            state["logs"].append(f"{datetime.now().strftime('%H:%M:%S')} - ✅ [PAPER TRADE EXECUTED] {order_msg}")
            return {"message": f"Paper Trade Successful: {order_msg}"}
            
        elif broker == "DHAN": 
            client.place_order(security_id="13", exchange_segment=client.NSE_FNO, transaction_type=client.BUY, quantity=req.lots * 15, order_type=client.MARKET, product_type=client.INTRADAY, price=0)
            
        elif broker == "ANGELONE": 
            client.placeOrder({"variety": "NORMAL", "tradingsymbol": f"{req.symbol}{req.strike}{req.option_type}", "symboltoken": "3045", "transactiontype": "BUY", "exchange": "NFO", "ordertype": "MARKET", "producttype": "INTRADAY", "duration": "DAY", "quantity": req.lots * 15})
