  ...
  <div id="mainTerminal" class="hidden max-w-7xl mx-auto p-4 space-y-6">
    <header class="flex flex-wrap justify-between items-center border-b border-slate-800 pb-4 gap-4">
      ...
      <div class="relative w-full md:w-80">
        <div class="flex items-center bg-slate-900 border border-slate-700 rounded-xl px-3 py-2">
          ...
          <input id="searchBar" type="text" placeholder="Search: NIFTY, Sensex, Crude..." oninput="handleSearch(this.value)" class="bg-transparent text-xs w-full focus:outline-none">
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div class="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-xl">
        <h2 class="text-sm font-bold text-sky-400 mb-3">Multi-Broker Gateway</h2>
                ...
                try:
                    ...
                    if state.get("ai_mode"):
                        if cp > pdh and cp > sma and rsi > 55 and state["algo_position"] != "CE":
                            ...
                            state["logs"].append(f"{datetime.now().strftime('%H:%M:%S')} - 🚀 🤖 [AI] Bullish Trend (RSI:{rsi}) -> BUY CE ({symbol})")
                        elif cp < pdl and cp < sma and rsi < 45 and state["algo_position"] != "PE":
                            state["algo_position"] = "PE"
                            state["logs"].append(f"{datetime.now().strftime('%H:%M:%S')} - 🚀 🤖 [AI] Bearish Trend (RSI:{rsi}) -> BUY PE ({symbol})")
                            
                    elif state.get("auto_trade") and not state.get("ai_mode"):
                        if cp > pdh and state["algo_position"] != "CE":
                            state["algo_position"] = "CE"
                            state["logs"].append(f"{datetime.now().strftime('%H:%M:%S')} - ⚡ [ALGO] PDH Breakout -> BUY CE ({symbol})")
                        elif cp < pdl and state["algo_position"] != "PE":
                            state["algo_position"] = "PE"
                            state["logs"].append(f"{datetime.now().strftime('%H:%M:%S')} - ⚡ [ALGO] PDL Breakdown -> BUY PE ({symbol})")
...
MASTER_SYMBOLS = [
    {"symbol": "^NSEI", "name": "NIFTY 50 (NSE)", "segment": "INDEX", "step": 50},
    {"symbol": "^NSEBANK", "name": "BANKNIFTY (NSE)", "segment": "INDEX", "step": 100},
    {"symbol": "NIFTY_FIN_SERVICE.NS", "name": "FINNIFTY (NSE)", "segment": "INDEX", "step": 50},
    {"symbol": "^BSESN", "name": "SENSEX (BSE)", "segment": "INDEX", "step": 100},
...
def market_data(symbol: str, username: str = Depends(get_current_user)):
    try:
        df = yf.Ticker(symbol).history(period="10d", interval="5m")
        if df.empty or len(df) < 20: raise ValueError("Failed to fetch live data.")
        df_d = yf.Ticker(symbol).history(period="5d", interval="1d")
        cp = round(float(df["Close"].iloc[-1]), 2)
        pdh = round(float(df_d.iloc[-2]["High"]), 2) if len(df_d) >= 2 else cp
        pdl = round(float(df_d.iloc[-2]["Low"]), 2) if len(df_d) >= 2 else cp
...
def buy_order(req: OrderReq, username: str = Depends(get_current_user)):
    ...
    try:
        if broker == "DHAN": client.place_order(security_id="13", exchange_segment=client.NSE_FNO, transaction_type=client.BUY, quantity=req.lots * 15, order_type=client.MARKET, product_type=client.INTRADAY, price=0)
