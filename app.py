...
# ---------------------------------------------------------
# BULLETPROOF IMPORTS (Prevents Render Crashes)
# ---------------------------------------------------------
try:
    import pandas as pd
except ImportError:
...
<body class="bg-slate-950 text-slate-100 min-h-screen font-sans antialiased overflow-x-hidden">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      <div class="bg-slate-900 border border-slate-800 p-4 rounded-2xl shadow-xl flex flex-col justify-between">
        ...
        <div class="grid grid-cols-2 gap-3 pt-6 md:pt-4">
          <button onclick="executeOptionOrder('CE')" class="bg-emerald-600 hover:bg-emerald-500 text-xs md:text-sm font-black py-4 md:py-3 rounded-xl transition shadow-lg shadow-emerald-600/30">BUY CALL</button>
          <button onclick="executeOptionOrder('PE')" class="bg-rose-600 hover:bg-rose-500 text-xs md:text-sm font-black py-4 md:py-3 rounded-xl transition shadow-lg shadow-rose-600/30">BUY PUT</button>
        </div>
      </div>
    </div>

    <!-- My Trends (Trade & Order History) -->
    <div class="bg-slate-900 border border-slate-800 p-4 rounded-2xl shadow-xl mt-4">
        <h2 class="text-sm font-bold text-indigo-400 mb-3 uppercase">📊 My Trends (History)</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <!-- Trade History -->
            <div class="bg-slate-800/50 p-3 rounded-xl border border-slate-700/50 h-48 flex flex-col">
                <h3 class="text-[10px] font-bold text-emerald-400 uppercase mb-2 border-b border-slate-700 pb-2">Trade History (Executed)</h3>
                <div id="tradeHistoryContainer" class="space-y-2 text-xs overflow-y-auto flex-1"></div>
            </div>
            <!-- Order History -->
            <div class="bg-slate-800/50 p-3 rounded-xl border border-slate-700/50 h-48 flex flex-col">
                <h3 class="text-[10px] font-bold text-slate-400 uppercase mb-2 border-b border-slate-700 pb-2">Order History (All Attempts)</h3>
                <div id="orderHistoryContainer" class="space-y-2 text-xs overflow-y-auto flex-1"></div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4">
      <div class="bg-slate-900 border border-slate-800 p-4 rounded-2xl shadow-xl">
        ...
        <select id="brokerSelect" onchange="renderBrokerForm()" class="w-full bg-slate-800 text-xs px-3 py-3 md:py-2 rounded-lg border border-slate-700 mb-3 font-semibold focus:outline-none">
          ...
          <option value="dhan">Dhan Broker</option>
      ...
      <div class="bg-slate-900 border border-slate-800 p-4 rounded-2xl shadow-xl flex flex-col justify-between">
        <div>
            ...
            <div class="mb-4">
                <label class="text-[10px] uppercase text-indigo-300">Send Command to AI</label>
  ...
  <script>
    ...
    function getTVSymbol(sym) {
        ...
        if (sym.endsWith("=F")) return "TVC:" + sym.replace("=F", ""); 
        return "NSE:" + sym; 
    }
    ...
    function renderTradingViewChart(sym, forceLayoutUpdate = false) {
      ...
      else grid.className = "grid grid-cols-1 md:grid-cols-2 gap-2 h-[800px] md:h-full w-full rounded-lg overflow-hidden transition-all"; 
    ...
    function renderBrokerForm() {
      ...
      else if (b === "dhan") f.innerHTML = `<input id="f_cid" placeholder="Dhan Client ID" class="w-full bg-slate-800 text-xs px-3 py-3 md:py-2 rounded-lg border border-slate-700 mb-2"><input id="f_tok" type="password" placeholder="Dhan Access Token" class="w-full bg-slate-800 text-xs px-3 py-3 md:py-2 rounded-lg border border-slate-700">`;
      else f.innerHTML = `<input id="f_cid" placeholder="Angel Client Code" class="w-full bg-slate-800 text-xs px-3 py-3 md:py-2 rounded-lg border border-slate-700 mb-2"><input id="f_key" placeholder="SmartAPI Key" class="w-full bg-slate-800 text-xs px-3 py-3 md:py-2 rounded-lg border border-slate-700 mb-2"><input id="f_pin" type="password" placeholder="MPIN" class="w-full bg-slate-800 text-xs px-3 py-3 md:py-2 rounded-lg border border-slate-700 mb-2"><input id="f_totp" type="password" placeholder="TOTP Secret" class="w-full bg-slate-800 text-xs px-3 py-3 md:py-2 rounded-lg border border-slate-700">`;
    }
    ...
    async function fetchStatus() {
      ...
      try{
        ...
        if(res.ok){
            ...
            document.getElementById("tradeHistoryContainer").innerHTML = d.trade_history.length === 0 ? '<div class="text-slate-500">No trades executed yet.</div>' : d.trade_history.map(t => `
                <div class="flex justify-between items-center p-2 rounded bg-slate-700/30 text-white mb-1">
                    <div class="flex flex-col"><span class="font-bold text-sky-400">${t.symbol}</span><span class="text-[9px] text-slate-400">${t.time} | ${t.type} x${t.lots}</span></div>
                    <span class="font-black text-emerald-400">@ ₹${t.price}</span>
                </div>
            `).join("");

            // Render Order History
            document.getElementById("orderHistoryContainer").innerHTML = d.order_history.length === 0 ? '<div class="text-slate-500">No orders placed yet.</div>' : d.order_history.map(o => `
                <div class="flex justify-between items-center p-2 rounded mb-1 ${o.status === 'SUCCESS' ? 'bg-emerald-900/20 border border-emerald-500/30' : 'bg-rose-900/20 border border-rose-500/30'}">
                    <div class="flex flex-col"><span class="font-bold text-slate-200">${o.symbol}</span><span class="text-[9px] text-slate-400">${o.time} | ${o.type}</span></div>
                    <div class="flex flex-col items-end"><span class="font-bold ${o.status === 'SUCCESS' ? 'text-emerald-400' : 'text-rose-400'}">${o.status}</span><span class="text-[8px] text-slate-400">${o.message}</span></div>
                </div>
            `).join("");
    ...
    async function sendAICommand() {
        ...
        alert((await res.json()).message); document.getElementById("aiCommandInput").value = ""; fetchStatus();
    }
        ...
        for username, state in TERMINAL_STATES.items():
            if (state.get("auto_trade") or state.get("ai_mode")) and state.get("is_connected"):
                ...
                try:
                    ...
                    # Helper to execute algo logic
                    def execute_algo_trade(opt_type, msg):
                        state["algo_position"] = opt_type
                        state["logs"].append(f"{t_now} - {msg}")
                        state["order_history"].insert(0, {"time": t_now, "symbol": symbol, "type": opt_type, "lots": state["lots"], "status": "SUCCESS", "message": "Algo Triggered"})
                        state["trade_history"].insert(0, {"time": t_now, "symbol": symbol, "type": opt_type, "lots": state["lots"], "price": cp})
                        if broker == "PAPER": state["paper_balance"] -= (cp * 15 * state["lots"]) # approx margin

                    if state.get("ai_mode"):
                        if cp > pdh and cp > sma and rsi > 55 and state["algo_position"] != "CE":
                            execute_algo_trade("CE", f"🚀 {pfx}🤖 [AI] Bullish (RSI:{rsi}) -> BUY CE ({symbol})")
                        elif cp < pdl and cp < sma and rsi < 45 and state["algo_position"] != "PE":
                            execute_algo_trade("PE", f"🚀 {pfx}🤖 [AI] Bearish (RSI:{rsi}) -> BUY PE ({symbol})")
                            
                    elif state.get("auto_trade") and not state.get("ai_mode"):
                        if cp > pdh and state["algo_position"] != "CE":
                            execute_algo_trade("CE", f"⚡ {pfx}[ALGO] Breakout -> BUY CE ({symbol})")
                        elif cp < pdl and state["algo_position"] != "PE":
                            execute_algo_trade("PE", f"⚡ {pfx}[ALGO] Breakdown -> BUY PE ({symbol})")
...
def market_data(symbol: str, username: str = Depends(get_current_user)):
    if yf is None: raise HTTPException(400, "yfinance library missing")
    try:
        df = yf.Ticker(symbol).history(period="10d", interval="5m")
...
def link(req: BrokerReq, username: str = Depends(get_current_user)):
    ...
    try:
        if req.broker == "paper":
            ...
            return {"message": "Paper Trading Activated!"}
            
        if req.broker == "dhan": 
            if dhanhq is None: raise Exception("dhanhq module not found")
            BROKER_INSTANCES[username] = dhanhq(req.client_id, req.access_token)
        elif req.broker == "angelone":
            if SmartConnect is None: raise Exception("smartapi-python module not found")
...
def buy_order(req: OrderReq, username: str = Depends(get_current_user)):
    ...
    try:
        if broker == "PAPER":
            ...
            state["logs"].append(f"{t_now} - ✅ [PAPER] Executed: {order_msg}")
            
            state["order_history"].insert(0, {"time": t_now, "symbol": req.symbol, "type": req.option_type, "lots": req.lots, "status": "SUCCESS", "message": "Virtual Match"})
            state["trade_history"].insert(0, {"time": t_now, "symbol": req.symbol, "type": req.option_type, "lots": req.lots, "price": "MKT"})
        ...
        state["order_history"].insert(0, {"time": t_now, "symbol": req.symbol, "type": req.option_type, "lots": req.lots, "status": "SUCCESS", "message": "Sent to Broker"})
        state["trade_history"].insert(0, {"time": t_now, "symbol": req.symbol, "type": req.option_type, "lots": req.lots, "price": "MKT"})
        state["logs"].append(f"{t_now} - [ORDER SUCCESS] {order_msg}")
        return {"message": f"Order sent: {order_msg}"}
    except Exception as e:
        state["order_history"].insert(0, {"time": t_now, "symbol": req.symbol, "type": req.option_type, "lots": req.lots, "status": "FAILED", "message": str(e)})
        state["logs"].append(f"{t_now} - [ORDER FAILED] {str(e)}")
